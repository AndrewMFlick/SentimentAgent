...
class AITool(BaseModel):
    ...
    aliases: list[str] = Field(
        default_factory=list, description="Alternative names/keywords for detection"
    )
    status: str = Field(
    ...
    approved_by: Optional[str] = Field(None, description="Admin username who approved")
    approved_at: Optional[datetime] = Field(None, description="Approval timestamp")
    rejected_by: Optional[str] = Field(None, description="Admin username who rejected")
    rejected_at: Optional[datetime] = Field(None, description="Rejection timestamp")
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Tool creation timestamp"
    )
