...
class AIAgent:
    ...
    def _extract_subreddits(self, question: str) -> list[str]:
        ...
        for subreddit in settings.subreddit_list:
            ...
            if (subreddit_lower in question_lower or 
                f"r/{subreddit_lower}" in question_lower or
                subreddit_lower.replace(" ", "") in question_lower):
        ...
        if context.get("summary"):
            ...
            for key, data in context["summary"].items():
                if key == "overall":
                    ...
                    prompt_parts.append(f"  Average sentiment: {data.get('avg_sentiment', 0):.3f}")
                else:
                    prompt_parts.append(f"\nr/{key}:")
                    ...
                    if "last_24h" in data:
                        ...
                        prompt_parts.append(f"    Positive: {stats.get('positive', 0)}, Negative: {stats.get('negative', 0)}, Neutral: {stats.get('neutral', 0)}")
                        prompt_parts.append(f"    Avg sentiment: {stats.get('avg_sentiment', 0):.3f}")
