...
from ..models.reanalysis import JobStatus, ReanalysisJobRequest
...
class ReanalysisService:
    ...
    async def _rate_limit_delay(self, batch_num: int = 0) -> None:
        ...
        if settings.reanalysis_batch_delay_ms > 0:
            ...
            logger.debug(
                f"Rate limit delay applied: {delay_seconds}s", batch_num=batch_num
            )

    async def _retry_with_backoff(
        self, operation, max_retries: int = None, operation_name: str = "operation"
    ):
        """
    ...
    async def _resolve_tool_aliases(self, tool_id: str) -> List[str]:
        ...
        try:
            ...
            logger.warning(
                "Failed to query aliases for tool", tool_id=tool_id, error=str(e)
            )
    ...
    async def check_active_jobs(self) -> int:
        ...
        try:
            result = list(
                self.jobs.query_items(query=query, enable_cross_partition_query=True)
            )
            ...
            logger.error("Failed to check active jobs", error=str(e), exc_info=True)
            # Fail-safe: assume no active jobs to avoid blocking
            return 0

    async def trigger_manual_reanalysis(
        self, job_request: ReanalysisJobRequest, triggered_by: str
    ) -> Dict[str, Any]:
        """
        ...
        if active_count > 0:
            raise ValueError(f"Cannot start job: {active_count} job(s) already active")
        ...
        if job_request.date_range:
            ...
            if date_range.get("start"):
                ...
                params.append({"name": "@start_ts", "value": int(start_dt.timestamp())})
            if date_range.get("end"):
                query_parts.append("AND c._ts <= @end_ts")
                end_dt = datetime.fromisoformat(
                    date_range["end"].replace("Z", "+00:00")
                )
                params.append({"name": "@end_ts", "value": int(end_dt.timestamp())})
        ...
        try:
            ...
            if result and len(result) > 0:
                first = result[0]
                total_count = (
                    int(first["count"]) if isinstance(first, dict) else int(first)
                )
            else:
                total_count = 0
        except Exception as e:
            logger.error(
                "Failed to count documents for reanalysis", error=str(e), exc_info=True
            )
        ...
        try:
            ...
            logger.error("Failed to create reanalysis job", error=str(e), exc_info=True)
    ...
    async def trigger_automatic_reanalysis(
        ...
        try:
            ...
            logger.error(
                "Failed to create automatic reanalysis job", error=str(e), exc_info=True
            )
            raise ValueError(f"Failed to create job: {str(e)}")
    ...
    async def update_tool_ids_after_merge(
        self, source_tool_ids: List[str], target_tool_id: str, merged_by: str
    ) -> Dict[str, int]:
        """
                ...
                for item in items:
                    ...
                    if changed:
                        try:
                            ...
                            item["last_analyzed_at"] = datetime.now(
                                timezone.utc
                            ).isoformat()
    ...
    async def process_reanalysis_job(
        self, job_id: str, sentiment_analyzer
    ) -> Dict[str, Any]:
        """
        ...
        # Load job
        try:
            job_doc = self.jobs.read_item(item=job_id, partition_key=job_id)
        except Exception as e:
            logger.error("Failed to load reanalysis job", job_id=job_id, error=str(e))
            raise ValueError(f"Job not found: {job_id}")

        # Validate state
        current_status = JobStatus(job_doc["status"])
        if current_status in {JobStatus.COMPLETED, JobStatus.FAILED}:
            raise ValueError(f"Job {job_id} already {current_status.value}")
        ...
        try:
            ...
            logger.error(
                "Failed to update job status to RUNNING", job_id=job_id, error=str(e)
            )
            raise
        ...
        try:
            ...
            while True:
                ...
                for item in items:
                    ...
                    try:
                        ...
                        if not content_id or not content_type:
                            logger.warning(
                                "Missing content_id or content_type", doc_id=doc_id
                            )
                        ...
                        try:
                            ...
                            if content_type == "post":
                                ...
                                content_doc = db_service.posts_container.read_item(
                                    item=content_id, partition_key=subreddit
                                )
                                # Post: title + content
                                ...
                                if not db_service.comments_container:
                                    logger.error("Comments container unavailable")
                                    uncategorized_count += 1
                                    processed_count += 1
                                    continue
                                ...
                                if post_id:
                                    content_doc = (
                                        db_service.comments_container.read_item(
                                            item=content_id, partition_key=post_id
                                        )
                                    )
                                else:
                                    ...
                                    results = list(
                                        db_service.comments_container.query_items(
                                            query=query,
                                            parameters=params,
                                            enable_cross_partition_query=True,
                                        )
                        ...
                        if not content or not content.strip():
                            logger.warning(
                                "Empty content", doc_id=doc_id, content_id=content_id
                            )
                            uncategorized_count += 1
                            processed_count += 1
                            continue
                        ...
                        # Detect tools
                        detected_tool_ids = sentiment_analyzer.detect_tools_in_content(
                            content
                        )
                ...
                if processed_count > 0:
                    ...
                    if elapsed_seconds > 0:
                        ...
                        if processing_rate > 0:
                            ...
                            job_doc["progress"]["estimated_time_remaining"] = int(
                                eta_seconds
                            )
                        else:
                ...
                job_doc["statistics"]["uncategorized_count"] = uncategorized_count
                ...
                try:
                    ...
                    logger.error(
                        "Failed to save checkpoint", job_id=job_id, error=str(e)
                    )
            ...
            sorted_tools = sorted(
                tools_detected.items(), key=lambda x: x[1], reverse=True
            )[:10]
            ...
            logger.error(
                "Reanalysis job failed", job_id=job_id, error=str(e), exc_info=True
            )
        ...
        try:
            ...
            logger.error("Failed to save final job state", job_id=job_id, error=str(e))

        return job_doc

    async def cancel_job(self, job_id: str, cancelled_by: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error("Job not found for cancellation", job_id=job_id, error=str(e))
        ...
        try:
            self.jobs.upsert_item(body=job_doc)
            logger.info(
                "Reanalysis job cancelled", job_id=job_id, cancelled_by=cancelled_by
            )
        except Exception as e:
