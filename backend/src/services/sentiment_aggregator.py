...
class SentimentAggregator:
    ...
    async def compute_daily_aggregates(self, date: str | None = None) -> list[dict]:
        """
        ...
        logger.info("Computing daily sentiment aggregates", date=date)
        ...
        aggregates = []
        for tool in tools:
            aggregate = await self.compute_aggregate_for_date(tool["id"], date)
            if aggregate:
                aggregates.append(aggregate)

        logger.info("Daily aggregates computed", date=date, tool_count=len(aggregates))

        return aggregates

    async def compute_aggregate_for_date(self, tool_id: str, date: str) -> dict | None:
        """
