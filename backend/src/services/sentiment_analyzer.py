...
class SentimentAnalyzer:
    ...
    def __init__(self):
        ...
        logger.info(f"Sentiment analyzer initialized with method: {settings.sentiment_method}")
    
    def analyze(self, content_id: str, content_type: str, subreddit: str, text: str) -> SentimentScore:
        """
        ...
        if settings.sentiment_method == "LLM" and self.llm_client:
            ...
            vader_result = self._analyze_with_vader(content_id, content_type, subreddit, text)
            ...
            if settings.use_llm_for_ambiguous and self.llm_client:
                if vader_result.confidence < 0.6:  # Low confidence threshold
                    logger.debug(f"Low confidence VADER result, using LLM fallback for {content_id}")
                    return self._analyze_with_llm(content_id, content_type, subreddit, text)
            
            return vader_result
    
    def _analyze_with_vader(self, content_id: str, content_type: str, subreddit: str, text: str) -> SentimentScore:
    ...
    def _analyze_with_llm(self, content_id: str, content_type: str, subreddit: str, text: str) -> SentimentScore:
...
Focus on the overall emotional tone regarding AI developer tools."""
            ...
            response = self.llm_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a sentiment analysis expert specializing in developer tool discussions."},
            ...
            return SentimentScore(
                ...
                positive_score=max(0, compound_score) if sentiment == "positive" else 0.0,
                negative_score=abs(min(0, compound_score)) if sentiment == "negative" else 0.0,
                neutral_score=1.0 - abs(compound_score) if sentiment == "neutral" else 0.0,
                analysis_method="LLM"
            )
    ...
    def _neutral_sentiment(self, content_id: str, content_type: str, subreddit: str, method: str) -> SentimentScore:
