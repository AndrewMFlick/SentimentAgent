...
async def get_tool_sentiment(
    ...
    try:
        ...
        if total > 0:
            positive_pct = (
                sentiment_data.get("positive_count", 0) / total
            ) * 100
            negative_pct = (
                sentiment_data.get("negative_count", 0) / total
            ) * 100
            neutral_pct = (
                sentiment_data.get("neutral_count", 0) / total
            ) * 100
        else:
            positive_pct = 0.0
            negative_pct = 0.0
...
async def compare_tools(
    ...
    tool_ids: str = Query(
        ...,
        description="Comma-separated tool IDs to compare"
    ),
    hours: Optional[int] = Query(
        default=None,
        description="Time window in hours (default: 24)"
    ...
    try:
        # Parse tool IDs
        parsed_tool_ids = [
            tid.strip() for tid in tool_ids.split(",") if tid.strip()
        ]
        
        if not parsed_tool_ids:
            raise HTTPException(
                status_code=400,
                detail="At least one tool_id is required"
            )
        ...
        tools_response = []
        for tool_id, sentiment_data in zip(
            parsed_tool_ids, sentiment_data_list
        ):
        ...
        for i in range(len(tools_response)):
            for j in range(i + 1, len(tools_response)):
                ...
                positive_delta = (
                    tool_a["positive_percentage"]
                    - tool_b["positive_percentage"]
                )
                negative_delta = (
                    tool_a["negative_percentage"]
                    - tool_b["negative_percentage"]
                )
                sentiment_delta = (
                    tool_a["avg_sentiment"] - tool_b["avg_sentiment"]
                )
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to compare tools"
        )
...
async def get_tool_timeseries(
    ...
    granularity: str = Query(
        default="daily",
        description="Time granularity (currently only 'daily')"
    )
):
    """
...
async def get_last_updated():
    ...
    try:
        ...
        logger.error(
            "Failed to get last updated timestamps",
            error=str(e),
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve last updated information"
        )
